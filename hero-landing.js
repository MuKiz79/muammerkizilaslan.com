...
if(!root||!hero)return;
const LEAD=4680,RAMP=350,GAP=200,words=t=>t.trim().split(/\s+/).filter(w=>/[\p{L}\d]/u.test(w)).length,hold=n=>450+220*n;
...
let timers=[],lead=null,active=false;
function clear(){for(const t of timers)clearTimeout(t);timers=[]}
function reset(){for(const p of lines)p.className=''}
function stop(){if(lead){clearTimeout(lead);lead=null}if(!active)return;active=false;clear();reset();root.hidden=true;hero.classList.remove('is-landing')}
function start(){if(active)return;if(lead){clearTimeout(lead);lead=null}active=true;reset();root.hidden=false;void root.offsetHeight;hero.classList.add('is-landing');
 ...
 timers.push(setTimeout(()=>{if(active)stop()},END))}
...
document.addEventListener('pointerdown',e=>{if(active&&e.target.closest&&e.target.closest('.hero'))stop()},true);
document.addEventListener('wheel',()=>{if(active)stop()},{passive:true,capture:true});
window.addEventListener('scroll',()=>{if(active&&scrollY>16)stop()},{passive:true});
document.addEventListener('keydown',e=>{if(active&&['Tab','Enter',' ','ArrowDown','ArrowUp','ArrowLeft','ArrowRight','PageDown'].includes(e.key))stop()});
